# DE FINAL PROJECT - SPAIN POPULATION, ECONOMIC SECTORS AND DEATHS. 
### By Javier Liarte 3erd IA+INF USJ 2025-2026
#### Data Engineering. Tutor: Victor Barceló


## INGESTION
To simulate a real ingestion process, the project includes the data_retrieval_simulation module, which is already uploaded to the GitHub repository. This module represents external data sources and is ingested in ingestion.py, where the raw data is loaded into the raw data layer without any modification.

### FILES

**death_causes_province.csv**  
Description:  
This file contains aggregated mortality data by province, year, sex, and cause of death. Each record represents the total number of deaths for a specific cause, geographic unit, and year.

Comments:  
This dataset is used as the main source for the mortality fact table. The raw file is preserved as-is during ingestion to ensure traceability between raw and transformed data.

**economic_sector_province.csv**  
Description:  
This file contains economic data by province and year, representing the percentage distribution of the population across different economic sectors.

Comments:  
The values are expressed as percentages rather than absolute values, making this dataset non-additive. This characteristic is considered during transformation and analysis to avoid incorrect aggregations.


**codauto_cpro.csv**  
Description:  
This file contains the official mapping between Autonomous Communities and Provinces, including their corresponding codes and names.

Comments:  
This dataset is used for future comparisons and analysis between Autonomous Communities, Provinces, and Municipalities.  
It was obtained from the Spanish National Statistics Institute (INE):  
https://www.ine.es/daco/daco42/codmun/cod_ccaa_provincia.htm  
and converted into CSV format with the assistance of AI. Since this file is small and static, it is included directly in the staging data layer instead of being dynamically ingested.



**pobmun20XX (2008–2024)**
Description:  
This file contains all the population in each municipality in Spain, including the genre.
...
## IMPORTANT: 
The developer knows that is safer to save the credentials in local variables. However in this project the security is no needed and it simplifies the connection.
...
Try the warehouse_connection_try.py to see if works.
